// Template puro do "Relatório de Prova" em PDF — A4 (SBOT: Poppins + Nunito
// Sans, paleta teal/navy). Este arquivo não depende de nada além de
// `./types` (só tipos, apagados na compilação), então roda tanto no
// navegador (src/utils/examReport.ts) quanto em Node, dentro da Cloud
// Function que gera o PDF no servidor (functions/src/index.ts, que importa
// uma cópia deste arquivo — ver functions/copy-shared.mjs). É a fonte única
// do HTML/CSS do relatório: nunca duplique esta lógica, só reaproveite.
...
// Corpo do script de renderização, executado dentro do documento do
// relatório (aba nova no navegador, ou a página headless da Cloud Function
// de geração no servidor). Escrito como string porque roda num `document`
// isolado (via window.open+document.write no navegador, ou page.setContent
// no Puppeteer) — não compartilha o bundle/módulos do chamador. `__ICON__`
// e `__DADOS__` são substituídos abaixo antes de injetar.
//
// O botão #print-btn só fica habilitado depois que todas as imagens e as
// fontes web (Poppins/Nunito Sans) terminam de carregar e as páginas são
// reajustadas com as medidas finais (ver waitUntilReadyToPrint) — a Cloud
// Function espera esse mesmo sinal antes de gerar o PDF headless, evitando
// duplicar a lógica de "está pronto pra imprimir?".
const REPORT_SCRIPT = `
(function () {
  "use strict";
  var ICON = "__ICON__";
...
// `iconUrl` precisa ser uma URL absoluta: no navegador vem de
// `window.location.origin` (ver openExamReport em src/utils/examReport.ts);
// na Cloud Function não existe `window`, então o chamador passa a URL
// pública fixa do ícone hospedado no Firebase Hosting.
export function buildReportDocument(data: ExamReportData, iconUrl: string): string {
...
// Monta o payload do relatório a partir de dados já buscados (no navegador,
// os que o ExamResultPage já tem em memória; na Cloud Function, os que
// generateExamReportPdf busca via firebase-admin) — nenhuma leitura extra
