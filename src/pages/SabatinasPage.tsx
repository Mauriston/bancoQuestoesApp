      ...
      {loading ? (
        ...
        groupedByDate.map(({ date, items }) => (
          <div key={date} className="space-y-3">
            <h2 className="text-sm font-bold text-slate-100">{formatIsoDateBr(date)}</h2>
                    ...
                    <div className="absolute top-2 right-2 z-10 flex items-center gap-1.5">
                      {pdfUrl && (
                        <button
                          ...
                          className="p-1.5 rounded-lg bg-slate-950/80 hover:bg-slate-800 text-slate-300 hover:text-slate-100 backdrop-blur-sm disabled:opacity-50"
                        >
    ...
    <div className="fixed inset-0 z-50 bg-slate-100/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 shadow-2xl space-y-4">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <h3 className="text-sm font-bold text-slate-100">{editingItem ? 'Editar' : 'Inserir'} Sabatina</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-3 text-xs">
          ...
          <div>
            ...
            <input required type="url" placeholder="https://docs.google.com/presentation/d/..." value={url} onChange={e => setUrl(e.target.value)} className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-slate-100" />
          </div>
