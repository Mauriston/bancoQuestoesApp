    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 bg-slate-900 hover:bg-slate-800 border border-slate-700 text-slate-200 hover:text-slate-100 px-4 py-2 rounded-xl text-xs font-semibold shadow-md transition-all self-start sm:self-auto"
        >
              ...
              <AreaChart data={evolutionData} margin={{ top: 8, right: 12, left: -18, bottom: 4 }}>
                <defs>
                  <linearGradient id="evoGradient" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#2F9C8C" stopOpacity={0.5} />
                    <stop offset="95%" stopColor="#2F9C8C" stopOpacity={0.02} />
                  </linearGradient>
                </defs>
                <CartesianGrid strokeDasharray="3 3" stroke="#2C3A47" />
                <XAxis dataKey="name" tick={{ fill: '#6B7680', fontSize: 10 }} />
                <YAxis domain={[0, 100]} tick={{ fill: '#6B7680', fontSize: 11 }} unit="%" />
                <Tooltip
                ...
                />
                <RechartsArea type="monotone" dataKey="score" stroke="#2F9C8C" strokeWidth={2.5} fill="url(#evoGradient)" />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </div>
          ...
          {areaChartData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={areaChartData} margin={{ top: 8, right: 12, left: -18, bottom: 4 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#2C3A47" />
                <XAxis dataKey="name" tick={{ fill: '#6B7680', fontSize: 10 }} interval={0} angle={-20} textAnchor="end" height={50} />
                <YAxis domain={[0, 100]} tick={{ fill: '#6B7680', fontSize: 11 }} unit="%" />
                <Tooltip
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-slate-900 border border-slate-800 border-l-4 border-l-cyan-500 rounded-2xl p-5 hover:border-slate-700 transition-colors">
          <div className="flex items-center gap-3.5">
            ...
            <div className="min-w-0">
              ...
              <p className="text-2xl font-black text-slate-100 mt-0.5 leading-none">{activeUsers.length}</p>
            </div>
          </div>
        </div>

        <div className="bg-slate-900 border border-slate-800 border-l-4 border-l-teal-500 rounded-2xl p-5 hover:border-slate-700 transition-colors">
          <div className="flex items-center gap-3.5">
            ...
            <div className="min-w-0">
              ...
              <p className="text-2xl font-black text-slate-100 mt-0.5 leading-none">
        ...
        <div className="bg-slate-900 border border-slate-800 border-l-4 border-l-emerald-500 rounded-2xl p-5 hover:border-slate-700 transition-colors">
          <div className="flex items-center gap-3.5">
        ...
        <div className="bg-slate-900 border border-slate-800 border-l-4 border-l-purple-500 rounded-2xl p-5 hover:border-slate-700 transition-colors">
          <div className="flex items-center gap-3.5">
            ...
            <div className="min-w-0">
              ...
              <p className="text-2xl font-black text-slate-100 mt-0.5 leading-none">{completedAttempts.length}</p>
            </div>
          </div>
        </div>
      </div>
              ...
              <tbody>
                {completedAttempts.slice(0, 10).map((att, i) => (
                  <tr key={att.id} className={`hover:bg-cyan-500/5 transition-colors ${i % 2 === 1 ? 'bg-slate-950/60' : ''}`}>
                    <td className="p-3 font-semibold text-slate-100 border-b border-slate-800/70">{att.examName || 'Simulado'}</td>
